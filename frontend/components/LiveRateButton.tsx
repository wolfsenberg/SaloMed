...
export default function LiveRateButton({ phpPerXlm, source, loading, onRefresh, className = '' }: Props) {
  ...
  const label = loading ? 'Updating' : hasRate ? rateSourceLabel(source) : 'Get rate';
  ...
  return (
    <button
      ...
      onClick={onRefresh}
      disabled={loading}
      className={`inline-flex items-center justify-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-semibold leading-5 whitespace-nowrap transition-colors disabled:opacity-60 ${
        live
